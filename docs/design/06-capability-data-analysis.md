...
实现 `run(task, emit, signal)`：把一个 Task 变成一次 headless agent 执行——组装提示词、经 ACP 拉起 agent 跑现有 data-analytics 能力、把执行过程与最终回复翻译成 TaskEvent 流。**它是翻译壳，不是分析器**：所有分析智能在 agent + skill + domains/ 里。
...
- 仓库现有资产：`skills/data-analytics/SKILL.md`、`domains/`、`knowledge-store/`（要求 event-catalog.json 已生成——M0 知识更新仍手动跑）
...
> **人格外置（2026-06-12 增补）**：严谨数据分析师人格定义在同目录 `persona.md`，runner 每次 run 现读拼接在提示词最前——调性格改文件即可，无需重建/重启。人格写法原则：行为规则可检验（数字溯源/事实推断分离/不过度解读等 7 条），配反例锚定语气，不写空泛形容词。缺失不挡执行。
